...
class mk_af_Mymodel(design_model, _af_inputs, _af_loss, _af_prep, _af_design, _af_utils):
    def __init__(self,
                 protocol="fixbb",
                 use_multimer=False,
                 use_templates=False,
    ...
    def _get_model(self, cfg, callback=None):
        ...
        a = self._args
        runner_embed = model.RunEmbed(cfg,
                                recycle_mode=a["recycle_mode"],
                                use_multimer=a["use_multimer"])

        runner_prdict = model.RunPredict(cfg,
                                      recycle_mode=a["recycle_mode"],
                                      use_multimer=a["use_multimer"])
        # setup function to get gradients
        def _model_embed(params, model_params, inputs, key):
            inputs["params"] = params
            opt = inputs["opt"]
            ...
            # OUTPUTS
            #######################################################################
            outputs = runner_embed.apply(model_params, key(), inputs)
        ...
        def _model_prdict(params, model_params, inputs, key):
            inputs["params"] = params
            opt = inputs["opt"]
            ...
            # OUTPUTS
            #######################################################################
            outputs = runner_prdict.apply(model_params, key(), inputs)
